...
- **training file 만들기** — 유전자 예측에 쓰는 종별 학습 파일(`.trn`)을 게놈 폴더에서
  바로 만듭니다. 폴더를 고르면 **가장 완성도 높은 어셈블리 하나를 알아서 골라** 학습시킵니다.
  chewBBACA 가 배포하는 것은 19개 종뿐이라, 그 밖의 종은 이 기능이 유일한 정규 경로입니다
- **스키마 내보내기·불러오기**
