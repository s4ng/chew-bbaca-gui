...
- [클라이언트 설정 복사] — §2 의 TOML 세 줄을 클립보드로. 손으로 조립할 일을 없앤다
- [토큰 재발급], MCP 사용 여부 토글, 자동 승인 토글
...
**아직 확인하지 않은 것 하나 — ChatGPT Desktop 으로 실제 접속.** 위 시험은 curl 과
`fetch` 로 한 것이라 프로토콜 적합성의 **필요조건**만 본 셈이다. 실제 클라이언트가
요구하는 헤더(`Accept: application/json, text/event-stream` 등)에서 막히면 그것이
`rmcp` 로 갈아탈 신호다. 다음 세션의 첫 확인 항목이다.

---

## 10. 남은 미검증 가정

1. **`http_headers` 에 `Authorization` 을 정적 값으로 넣는 것이 통하는가.**
   Codex 설정에는 `bearer_token_env_var` 가 따로 있어서 `Authorization` 만 특별
   취급될 여지가 있다. 통하지 않으면 사용자에게 환경변수를 설정하게 해야 하고,
   그러면 [설정 복사] 버튼의 출력이 달라진다. **5분이면 확인된다.**
2. **ChatGPT Desktop 이 쓰기 성격 도구를 플랜 제한 없이 부를 수 있는가.**
   웹 커넥터 쪽에는 Plus/Pro 읽기 전용 제한이 있다는 보고가 있다. 데스크톱 경로에도
   같은 제한이 있다면 실행 도구의 가치가 사람에 따라 달라진다.
